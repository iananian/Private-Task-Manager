...
# task의 양식은 다음 과 같음. (task, deadline, status)
tasks = []
...
# 이 아래는 chat-GPT draft임.
def update_listbox():
    listbox.delete(0, tk.END)
    for i, task in enumerate(tasks):
        listbox.insert(tk.END, f"Task {i}: {task[0]} | Deadline: {' '.join(map(str, task[1]))} | Status: {task[2]}")

def add_task_gui():
    task = entry_task.get()
    deadline = entry_deadline.get().split()
    try:
        deadline = [int(d) for d in deadline]
    except ValueError:
        messagebox.showwarning("입력 오류", "유효한 년 월 일을 입력하세요.")
        return
    ...
    if task:
        ...
        messagebox.showwarning("입력 오류", "Task를 작성하세요.")

def sort_by_order_gui():
    listbox.delete(0, tk.END)
    for i, task in enumerate(tasks):
        listbox.insert(tk.END, f"Task {i}: {task[0]} | Deadline: {' '.join(map(str, task[1]))} | Status: {task[2]}")

def sort_by_deadline_gui():
    sorted_tasks = sorted([task for task in tasks if not task[2]], key=lambda x: x[1])
...
def complete_task_gui():
    ...
    if not selected_task_index:
        messagebox.showwarning("선택 오류", "완료할 Task를 선택하세요.")
        return

    task_index = int(selected_task_index[0])
    if not tasks[task_index][2]:
        del tasks[task_index]
        update_listbox()
    else:
        messagebox.showinfo("이미 완료됨", "이미 완료된 Task입니다.")
...
# Task 입력을 위한 Entry
label_task = tk.Label(window, text="Task 내용:")
label_task.pack(pady=5)
entry_task = tk.Entry(window, width=30)
entry_task.pack(pady=5)

# Deadline 입력을 위한 Entry
label_deadline = tk.Label(window, text="Deadline (년 월 일):")
label_deadline.pack(pady=5)
entry_deadline = tk.Entry(window, width=30)
entry_deadline.pack(pady=5)

# 추가 버튼
button_add = tk.Button(window, text="Task 추가", command=add_task_gui)
button_add.pack(pady=10)

# Task 리스트 표시를 위한 Listbox
listbox = tk.Listbox(window, width=50, height=15)
listbox.pack(pady=10)

button_sort_order = tk.Button(window, text="Order순으로 정렬", command=sort_by_order_gui)
button_sort_order.pack(pady=5)

button_sort_deadline = tk.Button(window, text="Deadline순으로 정렬", command=sort_by_deadline_gui)
button_sort_deadline.pack(pady=5)

button_complete_task = tk.Button(window, text="Task 완료로 표시", command=complete_task_gui)
